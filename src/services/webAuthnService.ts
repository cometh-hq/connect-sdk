...
const createSigner = async ({
  ...
  try {
    ...
    if (webAuthnCredentials.publicKeyAlgorithm == -7) {
      signerAddress = await API.predictWebAuthnSignerAddress({
        publicKeyX,
        publicKeyY
      })
    ...
    return {
      publicKeyAlgorithm: webAuthnCredentials.publicKeyAlgorithm,
      publicKeyX,
      publicKeyY,
      publicKeyId,
      deviceData,
